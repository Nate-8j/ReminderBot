...
class UserHandlers:
    def __init__(self, repository):
        ...
        self.router.message.register(self.starting_message, CommandStart())
        self.router.message.register(self.command_help, Command('help'))
        self.router.message.register(self.examples, Command('examples'))
        self.router.message.register(self.add_reminder_type, F.text.lower() == "добавить")
        self.router.message.register(self.add_reminder_type, Command('new'))

    async def starting_message(self, message: types.Message):
        start_text = await self.repository.get_text_from_db('start_text')
        await message.answer(start_text, reply_markup=reply.add_kb)

    async def command_help(self, message: types.Message):
        text = await self.repository.get_text_from_db('command_help')
        await message.answer(text, parse_mode="HTML")

    async def examples(self, message: types.Message):
        text = await self.repository.get_text_from_db('examples')
        await message.answer(text, parse_mode="HTML")

    # 1) разовое 2) регулярное
    @staticmethod
...
#
#
# @usr_prv.callback_query(SimpleCalendarCallback.filter(), OneTimeStt.date)
...
#
#
# @usr_prv.callback_query(F.data.startswith('one-h'), OneTimeStt.hour)
...
#
#
# @usr_prv.callback_query(F.data.startswith('one-m'), OneTimeStt.minute)
...
#
#
# @usr_prv.message(OneTimeStt.txt)
...
#     )
#
#     job_id = await reminder_manager.add_onetime_reminder(
#         chat_id=message.chat.id,
#         text=txt,
...
#     )
#
#     job_id = await reminder_manager.add_regular_reminder(
#         chat_id=message.chat.id,
#         text=txt,
#         trigger=trigger
#     )
...
#     await state.clear()
#
#     schedulers = await repository.get_reminders(message.from_user.id)
...
# #
# #     await status_message.edit_text("\n".join(response) if response else "❌ ошибка")
